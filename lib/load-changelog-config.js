...
module.exports = async ({preset, config, parserOpts, writerOpts}) => {
  ...
  return mergeWith(
    {},
    !preset && !config && parserOpts ? {parserOpts} : {parserOpts: {...loadedConfig.parserOpts, ...parserOpts}},
    !preset && !config && writerOpts ? {writerOpts} : {writerOpts: {...loadedConfig.writerOpts, ...writerOpts}},
